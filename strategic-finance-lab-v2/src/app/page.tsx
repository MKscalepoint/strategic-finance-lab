    ...
    <div className="my-6 border border-mist bg-card/60 p-5">
      ...
      {scenario.summary && (
        <p className="text-xs text-slate mt-3 pt-3 border-t border-mist leading-relaxed">
          <span className="font-medium text-ink">{active.charAt(0).toUpperCase() + active.slice(1)}:</span> {scenario.summary}
        </p>
      )}
    </div>
  );
            ...
            {summaryStatus === "sent" ? (
              ...
              <>
                ...
                <button onClick={() => { setSummaryStatus("sending"); sendEmail(summaryEmail, "report", () => setSummaryStatus("sent"), () => setSummaryStatus("error")); }}
                  ...
                  className="w-full bg-accent text-paper text-xs py-2.5 hover:opacity-90 disabled:opacity-40 flex items-center justify-center gap-2">
