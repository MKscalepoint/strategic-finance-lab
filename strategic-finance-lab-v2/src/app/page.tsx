...
import { ArrowRight, Mail, RotateCcw, ChevronRight, Send, CheckCircle } from "lucide-react";
...
Please begin the analysis.`;
  ...
  async function streamResponse(msgs: Message[]) {
    ...
    try {
      ...
      if (
        ...
        accumulated.toLowerCase().includes("briefing document") ||
  ...
  async function exportExcel() {
    ...
    a.href = url;
    a.download = `scaler-analysis-${Date.now()}.xlsx`;
    a.click();
          ...
          <div className="max-w-2xl mx-auto text-center">
            ...
            >
              <p className="text-slate text-lg mb-4 leading-relaxed">
                Structural finance analysis for payments and fintech businesses.
              </p>
              <p className="text-slate text-base mb-12 leading-relaxed">
                Not a reporting tool. Not a forecast generator. A framework for
                understanding how your business actually behaves as it scales.
              </p>
            </div>
            ...
            >
              <div className="grid grid-cols-3 gap-8 text-left">
                {[
                  { n: "01", t: "Select a question", d: "Choose the scaling question most relevant to your current decision." },
                  { n: "02", t: "Describe your business", d: "Tell Scaler about your revenue model, stage, and what you are trying to understand." },
                  { n: "03", t: "Receive your analysis", d: "Scaler delivers a structured analysis. Enter your email at the end to receive the Word doc and financial model." },
                ].map((s) => (
                  <div key={s.n}>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="font-mono text-xs text-slate">0{q.id}</span>
                        <span className="text-xs text-accent font-medium tracking-wide uppercase">{q.label}</span>
                      </div>
                      <p className="font-display text-lg font-medium text-ink mb-2 leading-snug">{q.question}</p>
                      <p className="text-sm text-slate leading-relaxed">{q.description}</p>
                    </div>
                    <ChevronRight size={18} className="text-mist group-hover:text-accent transition-colors mt-1 flex-shrink-0" />
                  </div>
                </button>
              ))}
            </div>
        ...
        <main className="min-h-screen pt-24 pb-16 px-6">
          <div className="max-w-2xl mx-auto">
            ...
            >
              ...
              <p className="text-slate text-base">
                The more specific you are, the more precise the analysis.
              </p>
            </div>

            <div
              className="opacity-0 animate-fade-up mb-8"
            ...
            >
              ...
              <div>
                <label className="block text-sm font-medium text-ink mb-2">
                  Business context{" "}
                  <span className="text-slate font-normal">— what do you need us to know?</span>
                </label>
        ...
        <main className="pt-20 pb-0 min-h-screen flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-8">
            <div className="max-w-3xl mx-auto space-y-8">
              {messages.map((m, i) => (
                <div key={i} className={`animate-fade-in ${m.role === "user" ? "pl-12" : ""}`}>
                  {m.role === "user" ? (
                  ...
                  {emailStatus === "sent" ? (
                    <div className="flex items-center gap-2 mt-3">
                      ...
                      <p className="text-sm text-green-700">
                        Sent to <strong>{userEmail}</strong> — check your inbox.
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="border border-mist bg-white/80 px-3 py-2 text-sm text-ink placeholder-slate/50 focus:outline-none focus:border-accent transition-colors flex-1 max-w-xs"
                        />
                        <button
                          onClick={sendByEmail}
                          disabled={!userEmail.trim() || emailStatus === "sending"}
                          className="inline-flex items-center gap-2 bg-accent text-paper px-5 py-2 text-xs tracking-wide hover:bg-accent-light transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                        >
                          {emailStatus === "sending" ? (
                            <>Sending…</>
                          ) : (
                            <><Send size={12} /> Send to my inbox</>
                          )}
                        </button>
                      </div>
                      {emailStatus === "error" && (
            ...
            <div className="max-w-3xl mx-auto">
              <div className="flex gap-3 items-end">
                <textarea
                  ...
                  placeholder="Provide data, ask to go deeper, or respond to Scaler's questions…"
                  rows={2}
                  disabled={streaming}
